...
import { BlockUnblockUser, GetAllUsers, SoftDeleteUser } from "../services/AdminService/UserService";
...
const ManageUser = () => {
    ...
    const fetchUsers = async (search = "", role = "") => {
        try {
            setLoading(true);
            const userData = await GetAllUsers(search, role);
            setUsers(userData);
                                ...
                                filteredUsers.map((user) => (
                                    <tr key={user.id} className={`${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-50"} transition-colors`}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            >
                                                {user.role === 1 ? "Admin" : "Standard User"}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {user.role !== 1 && (
